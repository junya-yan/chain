...
やっていること:
 1. 4x4 のセルに切り分ける（コマ番号の重複と透かしを除いて 11 コマを選ぶ）
 ...
 4. 横一列のシートに組み直して PNG (RGBA) で書き出す
    ...
    python3 tools/extract-frames.py <元画像> <出力先>
    python3 tools/extract-frames.py art-source/tiger.png public/sprites/tiger.png

最後に出力される frameW/frameH/anchorX/anchorY は、そのまま
public/sprites/manifest.json に書き写す値。
"""
...
# (3,3) は元画像右下の "END" 透かしがキャラに重なるため使わない。11 コマで足りる。

# 塗りつぶしの許容色差。大きいほどよく抜けるが、キャラを削るリスクが上がる。
# トラの絵では 40 を超えると輪郭を突き抜けて体が分断される。
TOLERANCE = 34
# 地面の見本 1 色あたりの許容色差。見本を数十色持つので、単色で消していた頃の
# 値より大幅に小さくする。14 以上にすると靴から削れはじめる。
GROUND_MATCH = 10
# 出力する 1 コマのサイズ。
...
def remove_background(cell, tolerance=None):
...
def remove_ground_color(cell, rgba):
    """地面の色に近いピクセルを狙って消す。

    靴が地面に接して描かれているため、塗りつぶしだけでは地面がキャラと 1 つの
    連結成分になり、最大成分を残す処理では落ちない。
    地面の色はセルの左右端（キャラのいない位置）から採れるので、その代表色に
    近いものを消す。先にこれをやると地面がちぎれ、あとの最大成分の処理で
    残骸がまとめて消える。

    キャラで地面に最も近い色はオレンジの毛（色差 58）なので、
    GROUND_MATCH はそれより十分小さく取ること。

    地面は単色ではなく、ひび割れや小石で濃淡がある。代表色 1 つで消すと細い
    ひび割れが線として残るので、見本を量子化した「色の一覧」として持つ。

    見本を採る範囲は、しっぽが届かないセル下端に限る。中途半端な高さから採ると
    しっぽの色を地面と誤認して、しっぽごと消してしまう。
    """
    h = len(cell)
    w = len(cell[0])
    palette = set()
    for y in range(int(h * 0.86), h):
        for x in list(range(2, 30)) + list(range(w - 30, w - 2)):
            r, g, b, _ = cell[y][x]
            palette.add((r // 10, g // 10, b // 10))
    if not palette:
        return None, 0
    swatches = [(r * 10 + 5, g * 10 + 5, b * 10 + 5) for r, g, b in palette]
    ...
    for y in range(int(h * 0.5), h):
        for x in range(w):
            if not rgba[y][x][3]:
                continue
            c = cell[y][x]
            if any(dist(c, s) <= GROUND_MATCH for s in swatches):
                p = rgba[y][x]
                rgba[y][x] = (p[0], p[1], p[2], 0)
                removed += 1
    return len(swatches), removed
...
def main():
    ...
    for i, (r, c) in enumerate(CELLS):
        ...
        ground, cleaned = remove_ground_color(cell, rgba)
        ...
        x0, y0, x1, y1 = box
        print(
            "  コマ %2d: キャラ %dx%d (%d px)  地面色 %s 種を %d px 除去"
            % (i + 1, x1 - x0 + 1, y1 - y0 + 1, blob, ground, cleaned)
        )
